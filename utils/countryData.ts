...
// We use a comprehensive list of countries with their ISO codes and dial codes.

export interface Country {
  name: string;
  code: string;
  dialCode: string;
  flag: string; // Keep flag as emoji for fallback/metadata, but we'll use images in UI
}
...
export const getCountryFromTimezone = (): Country => {
  try {
    ...
    if (timezone.includes("Europe")) return countries.find(c => c.code === "DE") || countries[0];
    if (timezone.includes("Asia")) return countries.find(c => c.code === "BD") || countries[0];
    
    // Find by matching start of timezone name if possible
    const continent = timezone.split('/')[0];
    const city = timezone.split('/')[1];
    
    // If we have a city, try to find a country that might match (this is very rough)
    // For now, let's just stick to the most common ones or default.
  } catch (e) {
